...
\subsection{RQ1: Contract reliability}
...
up to test-sampling noise, showing a small certification margin.}
...
population-risk criterion. The repeated draws show three patterns.
First,
empirically tuned methods select policies whose \emph{true} risk is
out of contract in up to 49\% (empirical threshold tuning) and 57\%
(Bayesian optimization) of draws at these headline levels ---
...
$\delta$ everywhere. Second, the certified methods satisfy the
population-level bound with margin. At the headline levels, the full
optimizer's true risk exceeds $\alpha$ in at most 0.1\% of draws: below
$.003$ on HybridQA, CRAG, and ASQA, and one violating draw on QAMPARI.
The rate never exceeds 0.8\% anywhere on the sweep, an order of
magnitude inside the $\delta = 10\%$ budget. These low rates follow from the test,
...
absolute rate. The protocol's resolution also explains occasional
near-floor baseline entries, such as the utility router at
...
other levels in Figure~\ref{fig:sweep}). This result is specific to one
setting and is not a guarantee. On the
...
with a margin of 0.02--0.07 that shrinks with calibration size, matching
the finite-sample radius of the underlying test. Third, certification
need not increase cost: on CRAG the full optimizer is both the safest and
...
are not (CRAG, 28\%; QAMPARI, 43\%). Point estimates alone do not
distinguish these regimes.
...
\smallskip\noindent\textbf{RQ1 summary.}\par
On exact finite-population risk, uncertified tuners deploy
out-of-contract policies in up to 64\% of draws. Certification remains
within the $\delta$ budget at every tested contract level, also controls
query slices, and reports infeasible cases.

\subsection{RQ2: Cost-efficiency at matched contracts}
\label{sec:exp-cost}
...
$\alpha$. The relevant lower bound for \system's objective is the
\emph{matched oracle}, the cheapest hindsight per-query assignment
with test mean loss $\le \alpha$. It is unattainable because it requires
...
candidate, \system\ returns infeasible and falls back to best effort.
...
shipping a policy that cannot keep the promise. Lower values of
$\alpha$ therefore require more deferral, and the human-review load is
...
$\delta/4$ correction is too conservative to certify any candidate.

\smallskip\noindent\textbf{RQ2 summary.}\par
The certified policy is 6.9--20$\times$ cheaper than the strongest
fixed pipeline and costs 1.4--6.7$\times$ the matched hindsight oracle.
Deferral enables contracts down to $\alpha=0.1$, and the
intersection--union test certifies four-constraint contracts that the
Bonferroni procedure cannot certify.

\subsection{RQ3: Risk vouchers and composition}
...
telescoping path voucher (Proposition~\ref{prop:composition}) requires
no such assumption and is 10\% wider than the union bound.}
\label{tab:rewrites}
\centering
...
downgrade, consistent with evidence-preserving rewrites being safer.
The composed plan cuts cost 8.3$\times$ (1.74 to 0.21
...
$.026/.081/.105/.164$, sum 0.376) is 10\% wider than the union bound but
requires no interaction assumption. It covered the population chain
harm in every one of the 1000 redraws ($>$99.7\% at
...
\smallskip\noindent\textbf{RQ3 summary.}\par
The vouchers achieve at least 98.9\% repeated-draw coverage at a 95\%
nominal level, and their ordering follows observed harm. Telescoping
composition is 10\% wider than an invalid union bound but requires no
interaction assumption; selectivity conditioning provides a
per-stratum risk signal.

\subsection{RQ4: Knowledge drift detection and recovery}
\label{sec:exp-drift}
...
of $\alpha = 0.66$, and without monitoring no alarm is raised
...
alarms across 20 drift-free control streams. The per-stream Ville bound
is $\delta/2 = .05$, and a compliant stream's supermartingale tends to
decrease, yielding a lower observed false-alarm rate (confirmed
...
at every magnitude and is unaffected by the 1500-query compliant
prefix. The p90 delay reaches
...
\paragraph{Recalibration after an alarm}
...
$\to$ fixed-sequence recertification on that window $\to$ deployment of
the newly certified policy, which restores the post-drift guarantee.
The loop holds phase-2 violation to 0.819 versus 0.886
...
Proposition~\ref{prop:lifetime} allocates error across an unbounded
alarm--recalibrate loop. We evaluate it on a four-phase
...
certified policy's own 0.30 mCNY deployment cost. Including this cost,
at 100\% audit the certified system's total cost of
...
$r = 1 / 0.25 / 0.1$, no false alarms at any rate. The audit rate
therefore controls the tradeoff between reaction time and monitoring
spend.
...
(correct-and-fresh) risk jumps from $(.79,.74,.70,.70)$ to
...
oracle that recalibrates at the drift point. Freshness contracts differ
operationally from quality contracts:
staleness breaks every plan at once, and only detect-and-recertify ---
not conservative planning --- restores the guarantee.

\smallskip\noindent\textbf{RQ4 summary.}\par
The monitor detects and attributes drift with no observed false alarms;
its delays follow the bound under full and sampled auditing. Certified
recalibration reaches within 1\% of the drift-time oracle, and the
geometric schedule covers repeated alarms. Including audit cost reduces
the saving from 20$\times$ to 10--18$\times$.

\subsection{RQ5: Generalization across LLM families and
serving backends}
...
Cost savings may depend on a vendor's price sheet. We therefore hold
the retrieval ladder, sufficiency scorers, candidate families, and
certification procedure fixed, and change only the generator column across
...
backends: one hosted API platform and one local deployment). We observe
four results.
...
deployment exceeds $\alpha$ on all four families. (ii) \emph{The
...
top tiers are disproportionately expensive. The policy is selected from
data without vendor-specific rules. (iii)
...
GPU-seconds; the certification test does not depend on the units of the
cost column.

\smallskip\noindent\textbf{RQ5 summary.}\par
Across the four tested model families, no certified deployment violates
the population-risk target in 200 draws. Savings reach 16.6$\times$
where model-price spreads are wide, and the selected policy family
changes with the price structure.

\subsection{RQ6: Knowledge-source selection and scalability}
\label{sec:exp-scale}
...
retrieval-side: the retrieval order is table-first or text-first, and
the retrieval method is lexical or hybrid with reranking. GPU-priced
retrieval is included in cost. Estimated
...
selectivity-dependent crossover that source selection exploits. We
pass the chooser family (fixed paths plus selectivity-threshold rules)
to the certification layer. At $\alpha = 0.5$, it certifies a mixed policy at
0.337 mCNY per query --- 31\% below the strongest fixed path (0.492)
with realized test violation $0.481 \le \alpha$. Selectivity therefore
supports the same decisions as in a relational optimizer, including join
ordering and skipping an expensive operator for a strong predicate;
however, the statistical test, rather than the estimate, establishes
feasibility.
...
69-candidate space certifies in 3--14 ms; overall optimization overhead
...
generators. Increasing the candidate count does not affect validity but
can reduce test power. At
...
\smallskip\noindent\textbf{RQ6 summary.}\par
Selectivity-driven source selection reduces retrieval-side cost by 31\%
under the same certificate. Certification processes candidate spaces of
size $10^5$ in seconds without changing validity.
