...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig_paradigm_contrast}
\caption{From statistical certification to accounting. (a)~A plain RAG
pipeline carries no rate contract. (b)~Learn-then-Test certifies a plan
on a calibration sample and pays a confidence width forever; the width
grows with $|\mathcal{P}|$ and any level below the risk floor is
infeasible. (c)~An online e-process can raise an alarm and trigger
recertification, but the guarantee remains probabilistic and assumption
dependent. (d)~Ledger-gated execution replaces the certificate with a
deterministic gate $B_{t-1}\ge 1-\alpha$, an optimistic action LP over
plans plus declining, and an audited ledger update; validity is an
identity of the trajectory and cost is a pure learning problem.}
\label{fig:paradigm}
\end{figure*}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig_method_framework}
\caption{Overview of ledger-gated execution. A query arrives under an
evidence contract and a heterogeneous plan space that includes the
decline action $\bot$ (left). The ledger gate opens only when the
unspent allowance can absorb a worst-case loss, otherwise the arrival is
declined at zero contract loss (centre). When the gate is open an
optimistic action LP spends the allowance on cost, the chosen plan is
executed and audited, and the ledger is updated (right). Validity is
discharged by accounting; statistics serve cost alone.}
\label{fig:method}
\end{figure*}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig_controller_loop}
\caption{Per-arrival controller. The stream supplies the contract and
the plan catalog (left). The gate, the optimistic action LP and the
ledger update form a closed loop whose only feedback is the realised
allowance $B_t$ (centre). The service either answers or declines
(right). The three ingredients at the bottom are individually useless:
without the ledger the optimiser cannot be aggressive; without declining
strict contracts are infeasible; without randomisation the controller
cannot interpolate between plan risks.}
