...
Figure~\ref{fig:method} summarises the three stages of one arrival;
Figure~\ref{fig:controller} shows how they sit inside the online
controller.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig_method}
\caption{One arrival under \system.
(1)~A query arrives with an evidence contract $\alpha$ and a
heterogeneous plan space $\mathcal{P}\cup\{\bot\}$.
(2)~The ledger gate opens only when the unspent allowance can absorb a
worst-case violation, otherwise the system declines with loss zero --
yielding the pathwise identity $V_t\le\alpha t$ with no $\delta$ and no
dependence on $|\mathcal{P}|$.
(3)~When the gate is open an optimistic action LP mixes plans with
declining, the chosen action is audited, and the ledger and risk/cost
estimates are updated; the learning layer carries an
$O(\sqrt{P\log T/T})$ cost regret against the offline optimum.}
\label{fig:method}
\end{figure*}

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig_controller}
\caption{Architecture of the ledger-gated controller.
Inputs are the query stream, the contract level $\alpha$, the decline
price $c_\bot$ and the plan catalogue.
The centre stacks the three ingredients that the ablation shows are
jointly necessary: the ledger gate (validity), the optimistic action LP
over $\mathcal{P}\cup\{\bot\}$ (cost), and online estimate/audit
accounting (labels).
Outputs are a served answer or a human handoff, together with the
pathwise contract and a bill close to the offline optimum.
Unlike a calibration certificate, the controller never pays a
confidence width on queries it has already answered.}
